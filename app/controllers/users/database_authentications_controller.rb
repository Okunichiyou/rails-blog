class Users::DatabaseAuthenticationsController < ApplicationController
  ...
  def create
    form_params = params.expect(user_database_authentication_registration: %i[user_name password password_confirmation confirmation_token])
  ...
  def link_create
    form_params = params.expect(user_database_authentication_link: %i[password password_confirmation confirmation_token])
    @form = User::DatabaseAuthenticationLinkForm.new(
      current_user: current_user,
      **form_params
    )
