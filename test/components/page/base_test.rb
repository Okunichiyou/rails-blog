...
class PageBaseTest < ViewComponent::TestCase
  # テスト用の継承クラス
  class TestComponent < ::Page::Base
    def initialize
      @html_options = { class: [ "test" ] }
    end
  end

  class TestNoClassComponent < ::Page::Base
    def initialize
      @html_options = {}
    end
  end
  ...
  test "継承先クラスでbefore_renderが正しいdata-scopeを設定すること" do
    ...
    assert_equal "page-base-test-test-component", html_options[:"data-scope"]
  end
  ...
  test "継承先クラスで独自のクラスが設定されている時、before_renderが正しいclassを追加すること" do
    ...
    html_options = target.instance_variable_get(:@html_options)
    assert_includes html_options[:class], "page-base-test-test-component"
  end
  ...
  test "継承先クラスで独自のクラスが設定されていない時、before_renderが正しいclassを追加すること" do
    ...
    html_options = target.instance_variable_get(:@html_options)
    assert_includes html_options[:class], "page-base-test-test-no-class-component"
  end
